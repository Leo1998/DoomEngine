...
public class SimpleAssetCache implements AssetCache {
	...
	@Override public <T> void addToCache(AssetKey<T> key, T obj) {
		keyToAssetMap.put(key, obj);
	}

	@Override @SuppressWarnings("unchecked") public <T> T getFromCache(AssetKey<T> key) {
		return (T) keyToAssetMap.get(key);
	}

	@Override public boolean deleteFromCache(AssetKey<?> key) {
